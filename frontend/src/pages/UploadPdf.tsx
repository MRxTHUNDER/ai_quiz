...
export default function UploadPdf() {
  ...
  const handleGenerate = async () => {
    ...
    if (!quota?.canGenerateQuestions) {
      setStatus({
        type: "error",
        message: `Question generation limit reached. You have generated ${quota.questionsGeneratedInPeriod} questions in the last ${quota.cooldownDays} days. Maximum allowed is ${quota.maxQuestionsPerPeriod} questions per ${quota.cooldownDays} days.`,
      });
      return;
    }
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {quota?.hasUploadedPdfBefore && (
          <div className="mb-6 p-4 rounded-md bg-yellow-50 border border-yellow-200">
            ...
            <p className="text-sm text-yellow-700">
              You have already uploaded a PDF. PDF upload is only allowed once. You can still generate questions directly without uploading a PDF.
            </p>
            {quota.pdfUploadDate && (
              <p className="text-xs text-yellow-600 mt-1">
                PDF uploaded on: {new Date(quota.pdfUploadDate).toLocaleDateString()}
              </p>
            )}
          </div>
        )}
        ...
        >
          ...
          {quota?.canGenerateQuestions ? (
            <div className="text-sm text-gray-700">
              <p className="mb-1">
                You can generate <strong>{quota.questionsRemaining}</strong> more questions in this period.
              </p>
              <p className="text-xs text-gray-500">
                Generated: {quota.questionsGeneratedInPeriod} / {quota.maxQuestionsPerPeriod} questions (last {quota.cooldownDays} days)
              </p>
            </div>
          ) : (
            <div className="text-sm text-gray-700">
              <p className="mb-1">
                You have reached the limit of <strong>{quota.maxQuestionsPerPeriod} questions per {quota.cooldownDays} days</strong>.
              </p>
              {daysRemaining !== null && (
                <p className="mb-1">
                  You can generate questions again in <strong>{daysRemaining} days</strong>
                </p>
              )}
              <p className="text-xs text-gray-500">
                Next reset:{" "}
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-md">
          ...
          <ul className="text-sm text-gray-700 space-y-1">
            <li>• PDF upload: <strong>Once only</strong> (cannot upload again after first upload)</li>
            ...
            <li>• Question generation limit: <strong>{quota?.maxQuestionsPerPeriod} questions per {quota?.cooldownDays} days</strong></li>
          </ul>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <select
              ...
              disabled={!quota?.canGenerateQuestions || generating || !selectedExam}
              className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50"
            >
              <option value="">
                {selectedExam
          ...
          <div>
            ...
            <input
              ...
              disabled={
                !quota?.canGenerateQuestions || generating || selectedFile !== null
              }
          ...
          <div>
            ...
            <input
              ...
              disabled={!quota?.canUploadPdf || !quota?.canGenerateQuestions || generating}
          ...
          <div>
            ...
            <input
              ...
              type="number"
              min="1"
              max={quota?.questionsRemaining ? Math.min(50, quota.questionsRemaining) : 50}
              value={numQuestions}
              onChange={(e) => {
            ...
            />
            <p className="text-xs text-gray-500 mt-1">
              Default: 50 questions (max: {quota?.questionsRemaining ? Math.min(50, quota.questionsRemaining) : 50} remaining)
            </p>
          </div>
